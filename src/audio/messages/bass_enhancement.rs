...
impl BeacnSubMessage for BassEnhancement {
    ...
    fn get_message_maximum_version(&self) -> VersionNumber {
        if matches!(
            ...
            BassEnhancement::Enabled(_)
                | BassEnhancement::GetEnabled
                | BassEnhancement::Preset(_)
                | BassEnhancement::GetPreset
    ...
    fn to_beacn_key(&self, v: VersionNumber) -> [u8; 2] {
        if v < EQ_HEADPHONES_VERSION {
            ...
            match self {
                BassEnhancement::Enabled(_) | BassEnhancement::GetEnabled => [0x00, 0x00],
                BassEnhancement::Preset(_) | BassEnhancement::GetPreset => [0x01, 0x00],
                BassEnhancement::Amount(_) | BassEnhancement::GetAmount => [0x02, 0x00],
                _ => panic!("Unexpected Bass Enhancement Key"),
            }
        }
    }
    ...
    fn from_beacn(key: [u8; 2], value: BeacnValue, _: DeviceType, v: VersionNumber) -> Self {
        if v < EQ_HEADPHONES_VERSION {
            ...
            match key[0] {
                ...
                0x01 => Self::Preset(BassPreset::read_beacn_u32(&value)),
...
impl BassEnhancement {
    pub fn get_preset(preset: BassPreset, v: VersionNumber) -> Vec<Message> {
        if v < EQ_HEADPHONES_VERSION {
            match preset {
                Preset1 => vec![
            ...
            vec![Message::BassEnhancement(BassEnhancement::Preset(preset))]
        }
    }

    pub fn get_amount(amount: f32, v: VersionNumber) -> Vec<Message> {
        if v < EQ_HEADPHONES_VERSION {
            vec![
            ...
            vec![Message::BassEnhancement(BassEnhancement::Amount(
                BassAmount(amount),
            ))]
        }
    }
...
impl BassPreset {
    fn write_beacn_u32(&self) -> BeacnValue {
        let mut buf = [0; 4];
        LittleEndian::write_u32(&mut buf, *self as u8 as u32);
        buf
    }
    fn read_beacn_u32(buf: &BeacnValue) -> Self {
        debug!("Reading Bass Preset: {:?}", buf);

        let value = LittleEndian::read_u32(buf);
        for var in Self::iter() {
            if var as u8 as u32 == value {
                return var;
            }
        }
